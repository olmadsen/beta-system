...
long * AlloSO(long size) {
   long * start;
   /*printf("AlloSO: %i, %i\n",size, 4 + size/4 + 1);*/
   start = (long *) Allocate(4 + size / 4 + 1 );
   start[0] = -2;
   start[1] = 1;
...
long AlloC(int origin, long *proto, long SP) {
   ...
   start = (struct Component*)Allocate(10);
   start->Proto = -1;
   start->GCAttr = 0;
...
int AlloORR(long origin, long proto, long range, long obj, long off) {
    long start,i; 
/*    printf("AlloORR: range=%i, off=%i\n",range,off);*/
    start = Allocate(24 + range);
...
int AlloORRC(long origin, long proto, long range, long obj, long off) {
    long start,i; 
...
int AlloVR1(long *this, int off, int range) {
    ...
    oldHT = heapTop;
    start = Allocate(4 + range); /* too large */
    ((long *)start)[0] = -6; 
    ((long *)start)[3]=range;
    this[off / 4] = start;
}
...
void NewVR(long * dst, int off, int range){
...
void ExtVR(long * dst, int off, int rangeExt){
     ...
     if (srcKind <= -12)  head = 6; else head = 4;
     srcRange =  ((long *)src)[3];
     newRep = Allocate(head + rangeExt + srcRange);
     ((long *)newRep)[0] = srcKind;
     ((long *)newRep)[1] = 0;
     ((long *)newRep)[2] = 0;
...
void ExtRR(long * dst, int off, int rangeExt){
   ExtVR(dst,off,rangeExt);
}
...
long CopyCPP(long *struc, long *dummy) {
  ...
  /*printf("\nCopyCPP: struc=0x%x, dummy=0x%x, proto=0x%x\n",struc,dummy,struc[3]);*/
    ...
    * callBack[cbTop+1] = lui r9,  entry>>16
    * callBack[cbTop+2] = ori r9,r9, entry & 0xffff
    * callBack[cbTop+3] = lui r8,  start>>16
    * callBack[cbTop+4] = ori r8,r8, start & 0xffff
    * callBack[cbTop+5] = jr  r9
    */

   CallBack[cbTop] = (long)struc;
   CallBack[cbTop+1] = 0x3c000000 | (9 << 16) | (entry >> 16); 
   CallBack[cbTop+2] = 0x34000000 | (9 << 21) | (9 << 16) | (entry & 0xffff);
   CallBack[cbTop+3] = 0x3c000000 | (8 << 16) | (start >> 16); 
   CallBack[cbTop+4] = 0x34000000 | (8 << 21) | (8 << 16) | (start & 0xffff);
   CallBack[cbTop+5] = 0x8 | (9 << 21);
...
void CopySVR(long *src, long *dst, int off, int first, int last){
   ...
   srcKind =((long *)src)[0];
   if (srcKind <= -12)  head = 6; else head = 4;
   newRep = (long *)Allocate(range + head+1);
...
void NewRR(long * dst, int off, int range){
   NewVR(dst,off,range);
}

void MkTO(char *txt, long *obj, long off) {
   int i;
   int textObj,repObj;
...
void CopyT(char *txt, long *obj, long off){
   int i, repObj, range;
   /*printf("CopyT; obj=%x, off=%i, txt=%s\n", (long)obj, off, txt);*/
   range = strlen(txt);
   repObj = Allocate(4 + (range/4) +2);
...
void CopyVR(long sourceRep, long destObj, long off) {
  ...
  repObj = Allocate(96); /* OBS proper size must be used */
...
void CopyRR(long sourceRep, long destObj, long off) {
   CopyVR(sourceRep,destObj,off);
}

void CopySRR(long *src, long *dst, int off, int first, int last){
   CopySVR(src,dst,off,first,last);
}
...
void Qua(long *dstQuaProto, long *theCell, long *dstQuaOrigin, long *this) {
}
...
long prefixLevel(long * proto, long PC) {
  ...
  printf("prefixLevel(proto=0x%x, PC=0x%x\n", proto, PC);
  mEntry = proto[-1];
  PL = 1;
  thePL = 1;
...
long *dyn(long * obj, long SP, long LSC) {
   ...
   */
   ...
   if ((long)proto == -1) {
     ...
     SPglobal = callerComp->SPx;
     LSCglobal = callerComp->CallerLSC;
     printf("Dyn: return ((struct Component *)obj)->CallerObj;\n");
   ...
   SPoff = (codeStart[0] & 0xffff) | 0xffff0000;
   SPoff = - SPoff;
   SPx = SP + SPoff;
   LSCglobal= ((long *)SPx)[-1];
   if (PL == 1) {   
     printf("Dyn: caller =  (long *)((long *)SPx)[-2];\n");
     caller =  (long *)((long *)SPx)[-2];}
   else {
       /* check if return instruction is
	*  lw $30, offset($sp)
        *  sp = 29
        */
       printf("Dyn: CheckForNestedCaller\n");
       inst = ((long *)LSCglobal)[0];
       opCode = (inst >> 26) & 0x3f;
       destReg = (inst >> 16) & 0x1f;
       baseReg = (inst >> 21) & 0x1f;
       offset = inst & 0xffff;
       if ((opCode == 35) &&  
           (destReg == 30) &&
           (baseReg == 29)) {
	 caller = (long *)((long *)SPx)[offset / 4];
	 printf("offset=%i Caller=0x%x\n",offset,caller);
       } else {caller = obj; };
   };
...
long ExO(long jumpAdr,long * exitObj, long PC, long * this, long SP) {
  ...
  a = this; SPx = SP; LSCglobal = PC; /* missing */
  while (a != exitObj) {
...
void Susp(long * this, long dummy, long RA, long SPz){
   long first;
