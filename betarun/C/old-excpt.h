/*
 * Warning:  This file comes from an old version of Mingw32
 * and it probably doesn't work, but it does let us compile.
 * Here is an email exchange I had with Anders Norlander, the
 * author:

On Tue, 22 Feb 2000, Erik Corry wrote:
  
> Hi 
> 
> In the latest Mingw32 (2.95.2) there is a very rudimentary excpt.h file
> with nothing in it.  This replaces the excpt.h that used to
> be Mingw32 (2.8.1), which had a lot more definitions.  It had 
> the header:
[snip]   
> 
> Is it possible to put the old version in again?
  
It is possible, but support for structured exception
handling is more or less non-existant. That support would have
to be built into the compiler (and it is not part of standard C).
If you want to use that feature of win32 together with gcc you are
on your own, sorry.
