...
such thread may contain coroutines.In this paper, we assume that at
most one thread is executing
...
In figure \ref{coroutine} is shown four snapshots of the execution states
...
\item Figure (a) shows the initial situation after execution of {\tt
  merger.call()} in {\tt main} at the beginning of {\tt merger.Do}.
  \begin{itemize}
  \item The coroutines {\tt A} and {\tt M} have been generated.
  \item
    At most one coroutine at a time is executing. By convention the
    active coroutine is marked by a {\tt *}. The caller-link of the
    active coroutine refers to the method currently being executed. In
    (a), {\tt Merger*} refers to {\tt Do} which is currently
    executing.
  \item For suspended coroutines, the caller-link refers to the top
    ...
    {\tt A} and {\tt M} respectively.
  \end{itemize}
\item Figure (b) shows the situation where the first {\tt A.call()} in
  merge is being executed. This leads to the {\tt Do}-method of {\tt
  A} being invoked.  The {\tt Do}-method then calls compute. The
  snapshot is at the label {\tt A:} just before {\tt suspend} in this
  first invocation of {\tt compute}.
\item Figure (c) shows the situation after execution of {\tt suspend}
  in {\tt A} at the label {\tt L1:} in {\tt Merger.do}.  Execution of
...
\item Figure (d) shows the situation where {\tt M.call()} has been
   executed by {\tt Merger} ...
\item Figure (e) shows the situation at {\tt L2:} in {\tt Merger}.
   Both {\tt A} and {\tt M} are suspended.
...
two references. Consider Figure \ref{coroutine} (d), (e) and (f).

\begin{itemize}
\item The transition from (d) to (e) illustrates a suspend of {\tt M}.
   In (d) there is a caller-link from the object {\tt M} to the method
   ...
   seen in part (e) of the figure.

\item The transition from (e) to (f) illustrates a call of {\tt A}.
  Consider the caller-links from {\tt Merger*} to {\tt Merger.do} and
  the caller-link from {\tt A} to {\tt A.compute}. Execution of {\tt
  A.call()} implies that these two links are swapped. The result may
  be seen on part (f) of the figure.
