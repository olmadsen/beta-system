...
In BETA is possible to exit a nested scope of method invocations. To
implement this in CJ, a (complicated) structure of exceptions have to
be set up.
...
In CJ the only way to implement leave and restart is to use the
execption mechanism. At the point of {\tt }L, an exception handler is
set up to catch exceptions that may be casted by {\tt leave} and/or
{\tt restart}.  {\tt Leave} or {\tt restart} are then implemented by
casting the appropriate exceptions.
