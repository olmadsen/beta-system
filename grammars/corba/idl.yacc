...
%type <dcval>	array_declarator op_type_spec seq_head

%type <slval>	scoped_name opt_context at_least_one_string_literal
...
scoped_name
	  ...
	  IDENTIFIER
	{
	  ...
	  $$ = new UTL_StrList($1, new UTL_StrList($3, NULL));
	}
	| scoped_name
	  SCOPE_DELIMITOR
	{
...
literal
	...
	| TRUETOK
	{
	  $$ = idl_global->gen()->create_expr((boolean) TRUE,
					    AST_Expression::EV_bool);
	}
	| FALSETOK
	{
	  $$ = idl_global->gen()->create_expr((boolean) FALSE,
					    AST_Expression::EV_bool);
	}
	;
	...
	at_least_one_declarator
	{
	  ...
	  if (s != NULL && $1 != NULL && $3 != NULL) {
	    ...
	    while (!(l->is_done())) {
	      ...
	      if (d == NULL)
		continue;
	      t = idl_global->gen()->create_typedef(
			     d->compose(AST_Type::narrow_from_decl($1)),
			     d->name(), p);
	      (void) s->add_typedef(t);
	      l->next();
	    }
	    delete l;
	  }
...
simple_type_spec
	...
	| scoped_name
	{
	  ...
	  if (s == NULL)
	    s = idl_global->scopes()->next_to_top();
	  if (s != NULL)
...
char_type : CHAR { $$ = AST_Expression::EV_char; };

octet_type : OCTET { $$ = AST_Expression::EV_octet; };

boolean_type : BOOLEAN { $$ = AST_Expression::EV_bool; };

any_type : ANY { $$ = AST_Expression::EV_any; };
...
struct_type :
	...
	'}'
	{
	  ...
	  if (idl_global->scopes()->top() == NULL)
	    ...
	    $$ = AST_Structure::narrow_from_scope(idl_global->scopes()->top());
	    idl_global->scopes()->pop();
	  }
	}
	;
	...
	declarator
	{
	   ...
	   */
	    ...
	    $$ = idl_global->gen()->create_field(
			   $3->compose(AST_Type::narrow_from_decl($1)),
			   $3->name(),
			   idl_global->pragmas());
	}
	;
...
enum_type :
	...
	'}'
	{
	  ...
	  if (idl_global->scopes()->top() == NULL)
	    ...
	    $$ = AST_Enum::narrow_from_scope(idl_global->scopes()->top());
	    idl_global->scopes()->pop();
	  }
	}
	;
...
string_type_spec
	...
	'>'
	{
	  ...
	  if ($4 == NULL || $4->coerce(AST_Expression::EV_ulong) == NULL) {
	     ...
	     */
	    (void) idl_global->root()
		      ->add_string(AST_String::narrow_from_decl($$));
	  }
	}
	| string_head
	{
...
array_declarator :
	...
	at_least_one_array_dim
	{
	  ...
	  if ($3 != NULL) {
	      ...
	      */
	     (void) idl_global->root()
	     	       ->add_array(AST_Array::narrow_from_decl($$));
	  }
	}
	;
	  ...
	  if (s != NULL && $4 != NULL && $6 != NULL) {
		...
		continue;
	      a = idl_global->gen()->create_attribute(
                               $1,
			       d->compose(AST_Type::narrow_from_decl($4)),
			       d->name(),
			       p);
	      /*
	       * Add one attribute to the enclosing scope
	       */
	      (void) s->add_attribute(a);
	      l->next();
	    }
	    delete l;
	  }
...
operation :
	...
	{
	  UTL_Scope		*s = idl_global->scopes()->top();
	  UTL_ScopedName	*n = new UTL_ScopedName($4, NULL);
	  ...
	  if (s != NULL && $2 != NULL) {
	    o =
	      idl_global->gen()->
	            create_operation(AST_Type::narrow_from_decl($2),
				     $1,
				     n,
				     p);
	    (void) s->add_operation(o);
	  }
	  /*
	...
	opt_context
	{
	  idl_global->set_parse_state(IDL_GlobalData::PS_OpContextCompleted);
	  UTL_Scope		*s = idl_global->scopes()->top();
	  AST_Operation		*o = NULL;
	  ...
	  if (s != NULL && s->scope_node_type() == AST_Decl::NT_op) {
	    ...
	    if ($8 != NULL && o != NULL)
	      (void) o->add_exceptions($8);
	    if ($10 != NULL)
	      (void) o->add_context($10);
	  }
	  /*
	...
	declarator
	{
	  ...
	  if ($3 != NULL && $5 != NULL && s != NULL) {
	    a = idl_global->gen()->create_argument(
			$1,
			$5->compose(AST_Type::narrow_from_decl($3)),
			$5->name(),
                        p);
	    (void) s->add_argument(a);
	  }
	}	    
	;
