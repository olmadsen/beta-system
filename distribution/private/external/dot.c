...
static void DOThandleDelete (int handle)
{
...
void DOThandlePerformCompaction ()
{ int *newIndices = (int *) malloc (DOTlength*sizeof(int));
  ...
  for (i = 0; i < nextHandle; i++) 
    if (handleTable[i] >= 0) {
      ...
      if (handleTable[i] == -1) DOThandleDelete (i);
    }
...
static int DOTinsert (long objRef)
{ int inx;

  if (DOTTop == DOTLimit) {
    if (DOTneedsCompaction)
      return -1;
    else {
      /* Extend DOT */
...
int DOThandleInsert (long ObjRef, DOTonDelete onDelete)
{ int handle, DOTinx;

  if ((DOTinx = DOTinsert (ObjRef)) == -1)
    return -1;
