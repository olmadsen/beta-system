...
BOOL DrawTransparentBitmap(HWND hwnd, HBITMAP hBitmap, short xStart,
                           short yStart, int xDestPos, int yDestPos, int width,int height,COLORREF cTransparentColor)
{
	HDC hdc;
	BYTE f;
	
	hdc = GetDC( hwnd );
	f = DrawTransparentBitmapHDC(hdc,hBitmap,xStart,yStart,xDestPos,yDestPos,width,height,cTransparentColor);
	ReleaseDC(hwnd,hdc);
	return f;
}

BOOL DrawTransparentBitmapHDC(HDC hdc, HBITMAP hBitmap, int xStart,
                           int yStart, int xDestPos, int yDestPos, int width,int height,COLORREF cTransparentColor)
{
   COLORREF   cColor;
   ...
   res = BitBlt(hdc, xDestPos, yDestPos, ptSize.x, ptSize.y, hdcMem, xStart, yStart,
          SRCCOPY);
...
BOOL DrawTransparentBitmapFromMask
  (HWND hwnd, HBITMAP hBitmap, int xStart, int yStart, int xDestPos, 
   int yDestPos, int width, int height, HBITMAP hBitmapMask)
{
  HDC hdc;
  BOOL f;
  ...
  f = DrawTransparentBitmapFromMaskHDC(hdc,hBitmap,xStart,yStart,xDestPos,yDestPos,width,height,hBitmapMask);
...
BOOL DrawTransparentBitmapFromMaskHDC
  (HDC hdc, HBITMAP hBitmap, int xStart, int yStart, int xDestPos, 
   int yDestPos, int width, int height, HBITMAP hBitmapMask)
{
   COLORREF   cColor,cTransparentColor;
   HBITMAP    bmAndBack, bmAndObject, bmAndMem, bmSave;
   HBITMAP    bmBackOld, bmObjectOld, bmMemOld, bmSaveOld;
   HDC        hdcMem, hdcBack, hdcObject, hdcTemp, hdcSave;
   POINT      ptSize;
   ...
   if (width>(bm.bmWidth-xDestPos)) width=bm.bmWidth-xStart;
   if (height>(bm.bmHeight-yDestPos)) height=bm.bmHeight-yStart;
   ...
   //FGC
   SelectObject(hdcObject,hBitmapMask);
...
BOOL drawBitmap (HDC hdc, HBITMAP hbm, INT x, INT y,  
                 INT xDestPos, INT yDestPos)
{
    HDC       hdcBits;
    BITMAP    bm;
...
BOOL drawBitmapOnBitmap(HBITMAP hdst,INT dstx, INT dsty,HBITMAP hsrc,INT srcx,INT srcy,
			INT width,INT height,INT tc,HBITMAP hmsk)
{
	BOOL f;
	HDC hdcdst,hdcsrc;
	COLORREF tcolor;
	...
	switch(tc) {
	case -1:    /* draw as not transparent  */
		hdcdst = CreateCompatibleDC((HDC)0);
		hdcsrc = CreateCompatibleDC((HDC)0);
		if (!hdcdst || !hdcsrc) return FALSE;
		
		SelectObject(hdcsrc,hsrc);
		SelectObject(hdcdst,hdst);

		f = BitBlt(hdcdst,dstx,dsty,width,height,hdcsrc,srcx,srcy,SRCCOPY);

		DeleteDC(hdcdst);
		DeleteDC(hdcsrc);
		break;
	case -2:    /* draw transparent with mask */
	        if (!hmsk) return FALSE;
	        hdcdst = CreateCompatibleDC((HDC)0);

		if (!hdcdst || !hmsk ) return FALSE;
		SelectObject(hdcdst,hdst);

		f = DrawTransparentBitmapFromMaskHDC(hdcdst,hsrc,dstx,dsty,srcx,srcy,width,height,hmsk);

		DeleteDC(hdcdst);

		break;
	default:    /* use tc as transparent color */
		tcolor = (COLORREF) tc;
		hdcdst = CreateCompatibleDC((HDC)0);
		SelectObject(hdcdst,hdst);
		
		f = DrawTransparentBitmapHDC(hdcdst,hsrc,0,0,dstx,dsty,width,height,tcolor);
